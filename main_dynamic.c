...
int Pri_id_idx = 0;//指示当前Pri_id的下一个空位置
...
*
*/
size_t searchHashMap(uint8_t* L, size_t n)
{
    int hashValue = hashFun(L, n);
    size_t next_idx = hashMap[hashValue][0];
    for (int i=1; i<=next_idx; ++i)
    {
        printf("%d ", hashMap[hashValue][i]);
    }
    printf("\n");
}
...
void trapGen(S_T_i_w* ptr_T_i_w, const char* w)
{
    ...
    element_pow_zn(ptr_T_i_w->T2, pk_fs, r2);
...
void indexEnc(const char* w, uint32_t id)
{
    ...
    random_gen_L(L, sizeof(L));//随机生成224位L,自己实现
    ...
    element_random(tempZr);
    element_init_G1(C[C_idx].C_id_2, pairing);
    element_pow_zn(C[C_idx].C_id_2, g1, tempZr);//C_id_2=g^r_id

    for(int i=0; i<Pri_id_idx; ++i)//顺序遍历判断id是否存在
    {
        if(Pri_id[i].id == id)
        {
            w_idx = i;
    ...
    if(w_idx == -1)//id不存在
    {
        Pri_id[Pri_id_idx].id = id;
        memcpy(Pri_id[Pri_id_idx++].pt, L, sizeof(L));

        H_1(digest, (uint8_t*)&id, sizeof(id));
        element_from_hash(tempG1, digest, 32);
        element_pow_zn(tempG1, tempG1, sigma);//temp=H1(id)^sigma
        ...
        n = element_to_bytes(buff, tempGT);
        H_2(digestH2, buff, n);
        memcpy(C[C_idx].C_id_1, digestH2, sizeof(digestH2));
        free(buff);
    }
    else//id存在
    {
        memcpy(C[C_idx].C_id_1, Pri_id[w_idx].pt, sizeof(Pri_id[w_idx].pt));
        memcpy(Pri_id[w_idx].pt, L, sizeof(L));
    }

    //计算C_id_3
    element_mul(tempZr, sigma, tempZr);
    element_pow_zn(tempG1, g1, tempZr);
    element_pairing(tempGT, tempG1, pk_ss_2);
    element_pow_zn(tempGT, tempGT, sk_o_inv);
    buff = (uint8_t*)calloc(element_length_in_bytes(tempGT), sizeof(uint8_t));
    ...
    xor(digestH2, L, sizeof(digestH2));

    //C_id_3赋值
    memcpy(C[C_idx++].C_id_3, digestH2, sizeof(digestH2));
...
int serch_L_exist(uint8_t *L, size_t n)
{
    ...
    if (next_idx == 1 && !C[hashMap[hashValue][next_idx]].physdel)
        return hashMap[hashValue][next_idx];

    for (i=1; i<=next_idx; ++i)
    {
        idx = hashMap[hashValue][i];
        if (!C[idx].physdel && !memcmp(L, C[idx].C1, n))
            return idx;
    }
    return -1;
}

int search_L_exist_id(uint8_t *L, size_t n)
{
    for (int i=0; i<C_idx; ++i)
    {
        if (!C[i].physdel && !memcmp(L, C[i].C_id_1, n))
            return i;
    }
    return -1;
}
...
int indexDel(S_DT_id* pt_DT_id)
{
    ...
    int idx = -1;
	int result_num = 0;
	uint8_t * buff = (uint8_t*)calloc(element_length_in_bytes(tempGT2), sizeof(uint8_t));

	memcpy(L, pt_DT_id->DT_id_1, 28);

    while (true)
    {
        idx = search_L_exist_id(L, sizeof(L));

        if (idx != -1)//搜索C1==L
        {
            C[idx].Del_Tag = true;
            element_pairing(tempGT, C[idx].C_id_2, pt_DT_id->DT_id_2);
            element_pow_zn(tempGT, tempGT, sk_ss);//U1=tempGT
            //U2=C_id_3
            //下面是用户过程
            element_pow_zn(tempGT2, tempGT, pt_DT_id->s);

            n = element_to_bytes(buff, tempGT2);

            H_2(digestH2, buff, n);
            xor(digestH2, C[idx].C_id_3, sizeof(digestH2));

			++result_num;

            //计算出Pt是还要返回给SS
            memcpy(L, digestH2, 28);
            idx = -1;
        }
        else
        {
...
int search(S_T_fs_w* ptr_T_fs_w)
{
    ...
    bool first_interact = true;

    while (true)
    {
        if (!first_interact)
        {
            if (delete_first && C[j].Del_Tag)
            {
                element_set(C[pre].C2, C[j].C2);
                memcpy(C[pre].C3, digestH3, sizeof(C[pre].C3));
                C[j].physdel = true;//物理删除
            }

            else if (delete_first && !C[j].Del_Tag)
            {
                element_set(C[pre].C2, C[j].C2);
                memcpy(C[pre].C3, C[j].C3, sizeof(C[pre].C3));
                memcpy(C[pre].C_id_1, C[j].C_id_1, sizeof(C[pre].C_id_1));
                element_set(C[pre].C_id_2, C[j].C_id_2);
                memcpy(C[pre].C_id_3, C[j].C_id_3, sizeof(C[pre].C_id_3));
                C[pre].Del_Tag = false;
                memcpy(Pt0, L, sizeof(Pt0));
                C[j].physdel = true;
                delete_first = false;
            }
            else if (!delete_first && !C[j].Del_Tag)
            ...
            else if (!delete_first && C[j].Del_Tag && cur != 1)
            {
                memcpy(tempL, Pt0, sizeof(tempL));
                xor(tempL, L, sizeof(tempL));
                memset(tempH3, 0, 4);
                memcpy(tempH3+4, tempL, sizeof(tempL));
                memset(tempH3+4+28, 0, 32);
                xor(tempH3, C[pre].C3, sizeof(C[pre].C3));
                memcpy(C[pre].C3, tempH3, sizeof(tempH3));
                C[j].physdel = true;
            }
            else if (!delete_first && C[j].Del_Tag && cur == 1)
            {
                memcpy(C[j].C3, digestH3, sizeof(digestH3));
                delete_first = true;
                pre = j;
            }
        }
        ...
        if (idx != -1)//搜索C1==L
        {
            ...
            if (*(uint32_t*)digestH3 != 0xffffffff)
                ++result_num;
            ...
            //若Del_Tag==1, 返回J = digestH3
            if (C[j].Del_Tag)
            {
                uint32_t temp = ((*(uint32_t*)digestH3) ^ 0xffffffff);
                memcpy(digestH3, (uint8_t*)&temp, 4);
                memset(digestH3+4, 0, sizeof(digestH3)-4);
                xor(digestH3, C[idx].C3, sizeof(digestH3));
            }
...
void deleteTrap(S_DT_id* ptr_DT_id, uint32_t id)
{
    uint8_t digest[32];
    uint8_t* buff;
    size_t n;
    ...
    H_1(digest, (uint8_t*)&id, sizeof(id));
    element_from_hash(tempG1, digest, 32);
    element_pow_zn(tempG1, tempG1, sigma);//
    ...
    buff = (uint8_t*)calloc(element_length_in_bytes(tempGT), sizeof(uint8_t));
    n = element_to_bytes(buff, tempGT);
    H_2(ptr_DT_id->DT_id_1, buff, n);
    free(buff);

    element_init_Zr(ptr_DT_id->s, pairing);
    element_set(ptr_DT_id->s, s);

    element_mul(tempZr, ptr_DT_id->s, sk_o);
    element_div(tempZr, sigma, tempZr);
    element_init_G2(ptr_DT_id->DT_id_2, pairing);
    element_pow_zn(ptr_DT_id->DT_id_2, g2, tempZr);
...
void interact_search()
{
    ...
    S_DT_id* ptr_DT_id = (S_DT_id*)malloc(sizeof(S_DT_id));
    ...
    while (true)
    {
        ...
        if (!strncmp(input, "_quit", 5))
        {
            return ;
        }
        if (!strncmp(input, "_delete", 7))
        {
            fgets(input, 50, stdin);
            int id = atoi(input);
            deleteTrap(ptr_DT_id, id);
            indexDel(ptr_DT_id);

            for (int i=0; i<C_idx; ++i)
            {

                printf("%d ", C[i].Del_Tag);
            }
            printf("\n");
            printf("id:%d delete\n", id);
            element_clear(ptr_DT_id->DT_id_2);
            element_clear(ptr_DT_id->s);
...
*/
    ...
    for (i=0; i<7500; ++i)
    {
        if(hashMap[i][0] != 0)
        {
            printf("i:%d, num:%d    : ", i, hashMap[i][0]);

            for (int j=1; j<=hashMap[i][0]; ++j)
            {
    ...
    */
    index_mem_use += (sizeof(struct S_C) + element_length_in_bytes(C[0].C2)
                   + element_length_in_bytes(C[0].C_id_2)) * C_idx;
    /*
    struct S_C
{
    uint8_t C1[28];
